...
reg2 <- glm(Party ~ YOB + Income + HouseholdStatus + Q109244 + Q115611 + Q98197 + Q116881 + Q118232, data = train_new, family = binomial())
summary(reg2)

set.seed(121)
rf_4 <-  randomForest(Party ~ YOB + Income + HouseholdStatus + Q109244 + Q115611 + Q98197 + Q116881 + Q118232, data = train_new, ntree = 500, importance = TRUE)


submission <- data.frame(USER_ID = test_new$USER_ID)
submission$Predictions <- predict(rf_4, test_new)
write.csv(submission, file='v10_show_of_hands.csv', row.names = FALSE)